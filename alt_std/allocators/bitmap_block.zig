...
pub const BitmapBlockAllocator = struct {
    const Self = @This();
    ...
    ///
    /// See also `initAlloc`
    pub fn init(buffer: []u8, block_size: usize, n_blocks_desired: usize) BitmapBlockAllocator {
    ...
    pub fn initAlloc(backing_allocator: Allocator, block_size: usize, allocatable_size: usize) !BitmapBlockAllocator {
        std.debug.assert(block_size >= 1);
...
test "BitmapBlockAllocator" {
    const BBA = BitmapBlockAllocator;
    try std.testing.expectEqual(@as(usize, 2 * usize_bytes + 2 * 16), BBA.bufferSizeRequired(16, 2));
    try std.testing.expectEqual(@as(usize, 1024), BBA.bufferSizeRequired(16, 63));
    try std.testing.expectEqual(@as(usize, 4088), BBA.bufferSizeRequired(16, 253));

    var buffer: [1024]u8 = undefined;
    var bba = BBA.init(&buffer, 16, 63);
